...
class ModbusReader {
    ...
    constructor() {
        this.client = new (ModbusRTU as any)();
        ...
        this.parity = (process.env.MODBUS_PARITY || 'even') as 'none' | 'even' | 'odd';
        ...
        this.slaveIds = JSON.parse(process.env.MODBUS_SLAVES || '[1,2]');
    ...
    private toFloat32(regA: number, regB: number, format = 'ABCD'): number {
    ...
    private async readParameter(registerInfo: typeof REGISTERS.voltage): Promise<number> {
    ...
    async readEnergyData(slaveId: number): Promise<ModbusEnergyData> {
    ...
    async readAllDevices(): Promise<ModbusEnergyData[]> {
        const results: ModbusEnergyData[] = [];
